...
The current implementation uses a deterministic mock extractor so the project can run without API keys. The architecture is intentionally LLM-ready: the mock extractor can be replaced with an OpenAI, Azure OpenAI, or internal LLM provider while keeping the scenario router, schema registry, and exporters stable.
